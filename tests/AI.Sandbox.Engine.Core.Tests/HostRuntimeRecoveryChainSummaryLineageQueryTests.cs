...
public sealed class HostRuntimeRecoveryChainSummaryLineageQueryTests
{
...
public void IdsRevisionsTicksAndArgumentsAreValidated()
{
    ...
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => ProjectSummary(chain, expectedChainRevision: -1));
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => ProjectSummary(chain, projectedTick: -1));

    var projection = ChainSummaryProjection(chain);
        ...
        () => global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryChainQueryFlow.QueryLineage<
                ...
                CompletionPayload>(
                    ...
                    queriedTick: 0));
    Xunit.Assert.Throws<ArgumentException>(
        () => QueryLineage(projection, default));
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => QueryLineage(
            projection,
            chain.RootCheckpointId,
...
public void ProjectionRejectsStaleRevisionAndRegressedTick()
{
    ...
    AssertProjectionStatus(
        ...
        ProjectSummary(
            chain,
            projectedTick: chain.ValidatedTick - 1));
}

[Xunit.Fact]
public void ProjectionPreservesExactValidatedChainAndCompactBounds()
{
    var chain = SupersessionChain();
    ...
    Xunit.Assert.Same(chain, projection.Chain);
    Xunit.Assert.Equal(chain.ChainId, projection.ChainId);
    Xunit.Assert.Equal(chain.SupersessionCount, projection.SupersessionCount);
    Xunit.Assert.Equal(
        chain.SupersessionCount + 1,
        projection.CheckpointCount);
    Xunit.Assert.Equal(chain.RootCheckpointId, projection.RootCheckpointId);
    Xunit.Assert.Equal(
        chain.LatestCheckpointId,
        projection.LatestCheckpointId);
    Xunit.Assert.Equal(chain.Revision + 1, projection.Revision);
    Xunit.Assert.Equal(
...
[Xunit.Fact]
public void RootCheckpointQueryResolvesOutgoingLineage()
{
    var projection = ChainSummaryProjection();
    var chain = projection.Chain;
    var result = QueryLineage(
        projection,
        chain.RootCheckpointId,
        queriedTick: projection.ProjectedTick + 1);
    ...
    Xunit.Assert.Equal(
        ...
        result.Status);
    Xunit.Assert.Same(projection, result.Projection);
    Xunit.Assert.Same(chain.RootCheckpoint, query.Checkpoint);
    Xunit.Assert.Null(query.IncomingSupersession);
    Xunit.Assert.Same(
        chain.FirstSupersession,
        query.OutgoingSupersession);
    Xunit.Assert.Equal(0, query.CheckpointIndex);
    Xunit.Assert.True(query.IsRootCheckpoint);
    Xunit.Assert.False(query.IsLatestCheckpoint);
}

[Xunit.Fact]
public void LatestCheckpointQueryResolvesIncomingLineage()
{
    var projection = ChainSummaryProjection();
    var chain = projection.Chain;
    var result = QueryLineage(
    ...
    Xunit.Assert.Same(chain.LatestCheckpoint, query.Checkpoint);
    Xunit.Assert.Same(
        chain.LatestSupersession,
        query.IncomingSupersession);
    Xunit.Assert.Null(query.OutgoingSupersession);
    Xunit.Assert.Equal(chain.SupersessionCount, query.CheckpointIndex);
    Xunit.Assert.False(query.IsRootCheckpoint);
    Xunit.Assert.True(query.IsLatestCheckpoint);
    Xunit.Assert.Equal(projection.Revision + 1, query.Revision);
    Xunit.Assert.Equal(
...
[Xunit.Fact]
public void QueryRejectsStaleProjectionRevision()
{
    var projection = ChainSummaryProjection();
...
[Xunit.Fact]
public void QueryRejectsRegressedTick()
{
    var projection = ChainSummaryProjection();
...
[Xunit.Fact]
public void QueryRejectsUnknownCheckpoint()
{
    var projection = ChainSummaryProjection();
    ...
    AssertQueryStatus(
        ...
        QueryLineage(
            ...
            Id<
                global::AI.Sandbox.Engine.Core.HostRuntime
                    .HostRuntimeRecoveryCheckpointIdKind>(500099)));
}
...
public void ContractsRemainImmutableBoundedAndSideEffectFree()
{
        ...
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryCheckpointLineageQuery<
                ...
                CompletionPayload>>(
                    QueryLineage(
                        projection,
                        projection.LatestCheckpointId).Query);
    ...
    Xunit.Assert.Equal(
        ...
        256);
    Xunit.Assert.Same(projection.Chain, query.Projection.Chain);
...
private static void AssertProjectionStatus(
    global::AI.Sandbox.Engine.Core.HostRuntime
        .HostRuntimeRecoveryChainQueryStatus expected,
    global::AI.Sandbox.Engine.Core.HostRuntime
