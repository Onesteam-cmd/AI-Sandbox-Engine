...
public sealed class HostRuntimeRecoveryContinuousCollectionPairCrossCollectionRangeTests
{
...
public void IdsRevisionsTicksAndArgumentsAreValidated()
{
        ...
        () => global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryContinuousCollectionPairFlow.ProjectSummary<
                ...
                CompletionPayload>(
                    ...
                    projectedTick: continuity.ValidatedTick));
    ...
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => ProjectCollectionPair(continuity, projectedTick: -1));

    var summary = CollectionPairSummary(continuity);
    Xunit.Assert.Throws<ArgumentException>(
        () => global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryContinuousCollectionPairFlow.QueryRange<
                ...
                CompletionPayload>(
                    queryId: default,
                    collectionPairSummary: summary,
                    startCheckpointId: summary.StartCheckpointId,
                    endCheckpointId: summary.EndCheckpointId,
                    expectedCollectionPairSummaryRevision: summary.Revision,
                    queriedTick: summary.ProjectedTick));
    Xunit.Assert.Throws<ArgumentNullException>(
        () => global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryContinuousCollectionPairFlow.QueryRange<
                RequestPayload,
                ...
                CompletionPayload>(
                    ...
                    null!,
                    summary.StartCheckpointId,
                    summary.EndCheckpointId,
                    expectedCollectionPairSummaryRevision: 0,
                    queriedTick: 0));
    Xunit.Assert.Throws<ArgumentException>(
            ...
            endCheckpointId: default(
                global::AI.Sandbox.Engine.Core.Identifiers.Id<
                    global::AI.Sandbox.Engine.Core.HostRuntime
                        .HostRuntimeRecoveryCheckpointIdKind>)));
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => QueryCrossCollection(summary, expectedSummaryRevision: -1));
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => QueryCrossCollection(summary, queriedTick: -1));
}

[Xunit.Fact]
public void SummaryProjectionRejectsStaleRevisionAndRegressedTick()
{
    var continuity = CollectionContinuity(AdjacentCollectionContext());

    AssertCollectionPairSummaryStatus(
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryContinuousCollectionPairStatus
            .StaleContinuityRevision,
        ProjectCollectionPair(
            continuity,
            expectedContinuityRevision: continuity.Revision + 1));
    AssertCollectionPairSummaryStatus(
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryContinuousCollectionPairStatus
        ...
        ProjectCollectionPair(
            continuity,
            projectedTick: continuity.ValidatedTick - 1));
}

[Xunit.Fact]
public void SummaryProjectionRejectsSourceSummaryAndSupersessionMismatch()
{
    var context = AdjacentCollectionContext();
    var projection = AdjacentCollectionProjection(context.Selection);
    var otherContext = AdjacentCollectionContext(seed: 620500);
    var otherProjection = AdjacentCollectionProjection(otherContext.Selection);

    var sourceMismatch = ConstructNonPublic<
        global::AI.Sandbox.Engine.Core.HostRuntime
    ...
    AssertCollectionPairSummaryStatus(
        ...
        ProjectCollectionPair(sourceMismatch));
    ...
    var overlappingSelection = SyntheticAdjacentCollectionSelection(
        ...
        overlappingSummary,
        suffix: 620650);
    var overlappingProjection = AdjacentCollectionProjection(overlappingSelection);
    var overlappingContinuity = ConstructNonPublic<
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryMultiSequenceCheckpointRangeContinuityValidation<
                RequestPayload,
                RecoveryWorldState,
    ...
    AssertCollectionPairSummaryStatus(
        ...
        ProjectCollectionPair(overlappingContinuity));

    var wrongBoundaryContinuity = ConstructNonPublic<
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryMultiSequenceCheckpointRangeContinuityValidation<
                RequestPayload,
                RecoveryWorldState,
    ...
    AssertCollectionPairSummaryStatus(
        ...
        ProjectCollectionPair(wrongBoundaryContinuity));
}

[Xunit.Fact]
public void SummaryProjectionPreservesExactContinuousEvidence()
{
    var context = AdjacentCollectionContext();
    var continuity = CollectionContinuity(context);
    var result = ProjectCollectionPair(
        continuity,
        projectedTick: continuity.ValidatedTick + 2);
    ...
    Xunit.Assert.Equal(
        ...
        result.Status);
    Xunit.Assert.Same(continuity, result.Continuity);
    Xunit.Assert.Same(continuity, summary.Continuity);
    Xunit.Assert.Same(context.Summary, summary.RangeSummary);
    Xunit.Assert.Same(continuity.AdjacentCollection, summary.AdjacentCollection);
    Xunit.Assert.Same(context.SourceCollection, summary.SourceCollection);
    Xunit.Assert.Same(context.SourceProjection, summary.SourceProjection);
    Xunit.Assert.Same(context.SourceProjection.Chain, summary.Chain);
    Xunit.Assert.True(summary.ConnectsPreviousCollection);
    Xunit.Assert.False(summary.ConnectsNextCollection);
    Xunit.Assert.Same(context.Boundary, summary.ConnectingSupersession);
    Xunit.Assert.Same(context.PriorCheckpoint, summary.StartCheckpoint);
    Xunit.Assert.Same(context.SuccessorCheckpoint, summary.EndCheckpoint);
    Xunit.Assert.Equal(0, summary.StartSummaryIndex);
    Xunit.Assert.Equal(1, summary.EndSummaryIndex);
    Xunit.Assert.Equal(2, summary.SummaryCount);
    Xunit.Assert.Equal(4, summary.SequenceCount);
    Xunit.Assert.Equal(4, summary.PairCount);
    Xunit.Assert.Equal(8, summary.WindowCount);
    Xunit.Assert.Equal(2, summary.CheckpointCount);
    Xunit.Assert.Equal(1, summary.SupersessionCount);
    Xunit.Assert.True(summary.StartsAtSourceCollectionStart);
    Xunit.Assert.True(summary.EndsAtSourceCollectionEnd);
    Xunit.Assert.True(summary.StartsAtRoot);
    Xunit.Assert.True(summary.EndsAtLatest);
    Xunit.Assert.Equal(continuity.Revision + 1, summary.Revision);
    Xunit.Assert.Equal(continuity.ValidatedTick + 2, summary.ProjectedTick);
}

[Xunit.Fact]
public void CrossCollectionQueryRejectsStaleRevisionAndRegressedTick()
{
    var summary = CollectionPairSummary(
        CollectionContinuity(AdjacentCollectionContext()));

    AssertCrossCollectionQueryStatus(
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryContinuousCollectionPairStatus
    ...
    AssertCrossCollectionQueryStatus(
        ...
        QueryCrossCollection(summary, queriedTick: summary.ProjectedTick - 1));
}

[Xunit.Fact]
public void QueryRejectsMissingReversedAndSingleCollectionRanges()
{
    var summary = CollectionPairSummary(
        CollectionContinuity(AdjacentCollectionContext()));
    var missingId = Id<
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryCheckpointIdKind>(620099);

    AssertCrossCollectionQueryStatus(
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryContinuousCollectionPairStatus.RangeStartNotFound,
        QueryCrossCollection(summary, startCheckpointId: missingId));
    AssertCrossCollectionQueryStatus(
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryContinuousCollectionPairStatus.RangeEndNotFound,
        QueryCrossCollection(summary, endCheckpointId: missingId));
    AssertCrossCollectionQueryStatus(
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryContinuousCollectionPairStatus.RangeOrderInvalid,
        QueryCrossCollection(
            summary,
            startCheckpointId: summary.EndCheckpointId,
            endCheckpointId: summary.StartCheckpointId));
    AssertCrossCollectionQueryStatus(
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryContinuousCollectionPairStatus
...
[Xunit.Fact]
public void QueryPreservesExactCrossCollectionEvidence()
{
    var context = AdjacentCollectionContext();
    var summary = CollectionPairSummary(CollectionContinuity(context));
    var result = QueryCrossCollection(
        summary,
        queriedTick: summary.ProjectedTick + 2);
    ...
    Xunit.Assert.Equal(
        ...
        result.Status);
    Xunit.Assert.Same(summary, result.CollectionPairSummary);
    Xunit.Assert.Same(summary, query.CollectionPairSummary);
    Xunit.Assert.Same(context.SourceProjection.Chain, query.Chain);
    ...
    Xunit.Assert.Same(context.Boundary, query.ConnectingSupersession);
    Xunit.Assert.True(query.CrossesCollectionBoundary);
    Xunit.Assert.True(query.StartsAtCollectionPairStart);
    Xunit.Assert.True(query.EndsAtCollectionPairEnd);
    Xunit.Assert.Null(query.IncomingSupersession);
    Xunit.Assert.Null(query.OutgoingSupersession);
    Xunit.Assert.Equal(summary.Revision + 1, query.Revision);
    Xunit.Assert.Equal(summary.ProjectedTick + 2, query.QueriedTick);
}

[Xunit.Fact]
public void QueryCollectionsRemainImmutable()
{
    var summary = CollectionPairSummary(
        CollectionContinuity(AdjacentCollectionContext()));
    var query = Xunit.Assert.IsType<
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryCrossCollectionCheckpointRangeQuery<
                RequestPayload,
                RecoveryWorldState,
                CompletionPayload>>(QueryCrossCollection(summary).Query);
...
public void ContractsRemainBoundedAndSideEffectFree()
{
    ...
    Xunit.Assert.Same(summary, query.CollectionPairSummary);
    Xunit.Assert.True(query.CrossesCollectionBoundary);
    Xunit.Assert.Equal(summary.CheckpointCount, query.CheckpointCount);
}
