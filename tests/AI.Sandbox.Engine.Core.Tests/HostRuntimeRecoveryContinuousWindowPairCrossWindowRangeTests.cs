...
public sealed class HostRuntimeRecoveryContinuousWindowPairCrossWindowRangeTests
{
...
{
    var continuity = PreviousContinuity();
    ...
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => ProjectPair(continuity, expectedContinuityRevision: -1));
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => ProjectPair(continuity, projectedTick: -1));

    var pair = PairSummary(continuity);
    Xunit.Assert.Throws<ArgumentException>(
        () => global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryContinuousWindowPairFlow.QueryRange<
                ...
                CompletionPayload>(
                    ...
                    queriedTick: pair.ProjectedTick));
    Xunit.Assert.Throws<ArgumentNullException>(
        () => global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryContinuousWindowPairFlow.QueryRange<
                RequestPayload,
                ...
                CompletionPayload>(
                    ...
                    null!,
                    pair.StartCheckpointId,
                    pair.EndCheckpointId,
                    expectedPairSummaryRevision: 0,
                    queriedTick: 0));
    Xunit.Assert.Throws<ArgumentException>(
        () => QueryCrossWindow(
            pair,
            ...
            endCheckpointId: default(
                global::AI.Sandbox.Engine.Core.Identifiers.Id<
                    global::AI.Sandbox.Engine.Core.HostRuntime
                        .HostRuntimeRecoveryCheckpointIdKind>)));
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => QueryCrossWindow(pair, expectedPairSummaryRevision: -1));
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => QueryCrossWindow(pair, queriedTick: -1));
}

[Xunit.Fact]
public void PairProjectionRejectsStaleRevisionAndRegressedTick()
{
    var continuity = PreviousContinuity();
...
[Xunit.Fact]
public void PreviousPairSummaryPreservesExactContinuousEvidence()
{
    var continuity = PreviousContinuity();
    var result = ProjectPair(
        continuity,
        projectedTick: continuity.ValidatedTick + 1);
    var pair = Xunit.Assert.IsType<
        global::AI.Sandbox.Engine.Core.HostRuntime
    ...
    Xunit.Assert.Same(continuity, pair.Continuity);
    Xunit.Assert.Same(continuity.Summary, pair.RangeSummary);
    Xunit.Assert.Same(continuity.AdjacentWindow, pair.AdjacentWindow);
    Xunit.Assert.Same(continuity.ConnectingSupersession, pair.ConnectingSupersession);
    Xunit.Assert.True(pair.ConnectsPreviousWindow);
    Xunit.Assert.False(pair.ConnectsNextWindow);
    Xunit.Assert.Same(pair.AdjacentWindow.StartCheckpoint, pair.StartCheckpoint);
    Xunit.Assert.Same(pair.RangeSummary.EndCheckpoint, pair.EndCheckpoint);
    Xunit.Assert.Same(pair.AdjacentWindow.EndCheckpoint, pair.ConnectingPriorCheckpoint);
    Xunit.Assert.Same(pair.RangeSummary.StartCheckpoint, pair.ConnectingSuccessorCheckpoint);
    Xunit.Assert.Equal(2, pair.CheckpointCount);
    Xunit.Assert.Equal(1, pair.SupersessionCount);
    Xunit.Assert.True(pair.StartsAtRoot);
    Xunit.Assert.True(pair.EndsAtLatest);
    Xunit.Assert.Equal(continuity.Revision + 1, pair.Revision);
    Xunit.Assert.Equal(continuity.ValidatedTick + 1, pair.ProjectedTick);
}

[Xunit.Fact]
public void NextPairSummaryPreservesExactContinuousEvidence()
{
    var continuity = NextContinuity();
    var pair = PairSummary(continuity);

    Xunit.Assert.False(pair.ConnectsPreviousWindow);
    Xunit.Assert.True(pair.ConnectsNextWindow);
    Xunit.Assert.Same(pair.RangeSummary.StartCheckpoint, pair.StartCheckpoint);
    Xunit.Assert.Same(pair.AdjacentWindow.EndCheckpoint, pair.EndCheckpoint);
    Xunit.Assert.Same(pair.RangeSummary.EndCheckpoint, pair.ConnectingPriorCheckpoint);
    Xunit.Assert.Same(pair.AdjacentWindow.StartCheckpoint, pair.ConnectingSuccessorCheckpoint);
    Xunit.Assert.Same(pair.Chain.FirstSupersession, pair.ConnectingSupersession);
}

[Xunit.Fact]
public void CrossWindowQueryRejectsStaleRevisionAndRegressedTick()
{
    ...
    AssertQueryStatus(
        ...
        QueryCrossWindow(
            pair,
            expectedPairSummaryRevision: pair.Revision + 1));
    AssertQueryStatus(
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryContinuousWindowPairStatus
            .CrossWindowRangeQueryTickRegressed,
        QueryCrossWindow(pair, queriedTick: pair.ProjectedTick - 1));
}

[Xunit.Fact]
public void QueryRejectsMissingReversedAndSingleWindowRanges()
{
    var pair = PairSummary(PreviousContinuity());
    var missingId = Id<
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryCheckpointIdKind>(540099);

    AssertQueryStatus(
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryContinuousWindowPairStatus
            .RangeStartNotFound,
        QueryCrossWindow(pair, startCheckpointId: missingId));
    AssertQueryStatus(
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryContinuousWindowPairStatus
            .RangeEndNotFound,
        QueryCrossWindow(pair, endCheckpointId: missingId));
    AssertQueryStatus(
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryContinuousWindowPairStatus
            .RangeOrderInvalid,
        QueryCrossWindow(
            pair,
            startCheckpointId: pair.EndCheckpointId,
            endCheckpointId: pair.StartCheckpointId));
    AssertQueryStatus(
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryContinuousWindowPairStatus
...
[Xunit.Fact]
public void PreviousPairQueryPreservesExactCrossBoundaryEvidence()
{
    var pair = PairSummary(PreviousContinuity());
    var result = QueryCrossWindow(pair, queriedTick: pair.ProjectedTick + 1);
    ...
    Xunit.Assert.Equal(
        ...
        result.Status);
    Xunit.Assert.Same(pair, result.PairSummary);
    Xunit.Assert.Same(pair, query.PairSummary);
    ...
    Xunit.Assert.Single(query.Supersessions);
    Xunit.Assert.Same(pair.StartCheckpoint, query.StartCheckpoint);
    Xunit.Assert.Same(pair.EndCheckpoint, query.EndCheckpoint);
    Xunit.Assert.Same(pair.ConnectingSupersession, query.ConnectingSupersession);
    Xunit.Assert.True(query.CrossesWindowBoundary);
    Xunit.Assert.Null(query.IncomingSupersession);
    Xunit.Assert.Null(query.OutgoingSupersession);
    Xunit.Assert.Equal(pair.Revision + 1, query.Revision);
    Xunit.Assert.Equal(pair.ProjectedTick + 1, query.QueriedTick);
}

[Xunit.Fact]
public void NextPairQueryPreservesExactCrossBoundaryEvidence()
{
    var pair = PairSummary(NextContinuity());
    var query = Xunit.Assert.IsType<
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryCrossWindowCheckpointRangeQuery<
                RequestPayload,
                RecoveryWorldState,
                CompletionPayload>>(QueryCrossWindow(pair).Query);

    Xunit.Assert.True(pair.ConnectsNextWindow);
    Xunit.Assert.Same(pair.RangeSummary.StartCheckpoint, query.StartCheckpoint);
    Xunit.Assert.Same(pair.AdjacentWindow.EndCheckpoint, query.EndCheckpoint);
    Xunit.Assert.Same(pair.Chain.FirstSupersession, query.ConnectingSupersession);
}
...
private static void AssertPairStatus(
    global::AI.Sandbox.Engine.Core.HostRuntime
        .HostRuntimeRecoveryContinuousWindowPairStatus expected,
    global::AI.Sandbox.Engine.Core.HostRuntime
...
private static void AssertQueryStatus(
    global::AI.Sandbox.Engine.Core.HostRuntime
        .HostRuntimeRecoveryContinuousWindowPairStatus expected,
    global::AI.Sandbox.Engine.Core.HostRuntime
