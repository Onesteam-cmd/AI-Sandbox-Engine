...
public sealed class HostRuntimeRecoveryMultiWindowRangeSummaryAdjacentSequenceTests
{
...
public void IdsRevisionsTicksCountsAndArgumentsAreValidated()
{
    ...
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => ProjectMultiWindowSummary(range, expectedRangeRevision: -1));
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => ProjectMultiWindowSummary(range, projectedTick: -1));

    var summary = MultiWindowSummary(range);
    Xunit.Assert.Throws<ArgumentException>(
        () => global::AI.Sandbox.Engine.Core.HostRuntime
        ...
        () => global::AI.Sandbox.Engine.Core.HostRuntime
            ...
            .SelectPreviousSequence<
                ...
                CompletionPayload>(
                    ...
                    selectedTick: 0));
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => SelectPreviousSequence(summary, pairCount: 0));
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => SelectPreviousSequence(summary, expectedSummaryRevision: -1));
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => SelectPreviousSequence(summary, selectedTick: -1));
}

[Xunit.Fact]
public void MultiWindowSummaryRejectsStaleRevisionAndRegressedTick()
{
    var range = MultiWindowRange();

    AssertMultiWindowSummaryStatus(
        global::AI.Sandbox.Engine.Core.HostRuntime
...
[Xunit.Fact]
public void MultiWindowSummaryPreservesExactCompactEvidence()
{
    var range = MultiWindowRange();
    var result = ProjectMultiWindowSummary(
        range,
    ...
    Xunit.Assert.Equal(range.StartPairIndex, summary.StartPairIndex);
    Xunit.Assert.Equal(range.EndPairIndex, summary.EndPairIndex);
    ...
    Xunit.Assert.Equal(range.PairCount, summary.PairCount);
    Xunit.Assert.Equal(range.CrossedBoundaryCount, summary.CrossedBoundaryCount);
    Xunit.Assert.Equal(range.CrossedWindowCount, summary.CrossedWindowCount);
...
{
    var summary = MultiWindowSummary(MultiWindowRange());

    AssertAdjacentSequenceStatus(
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryMultiWindowCheckpointRangeSummaryStatus
    ...
    AssertAdjacentSequenceStatus(
        ...
        SelectPreviousSequence(
            ...
            pairCount:
                global::AI.Sandbox.Engine.Core.HostRuntime
                    .HostRuntimeRecoveryMultiWindowCheckpointRangeSummaryFlow
                    .MaximumAdjacentSequencePairCount + 1));
}

[Xunit.Fact]
public void PreviousSelectionRejectsMissingAdjacentSequence()
{
    var summary = MultiWindowSummary(MultiWindowRange());
    var result = SelectPreviousSequence(summary);
...
[Xunit.Fact]
public void NextSelectionRejectsMissingAdjacentSequence()
{
    var summary = MultiWindowSummary(MultiWindowRange());
    var result = SelectNextSequence(summary);
...
{
    var summary = MultiWindowSummary(MultiWindowRange());
    var previous = SelectPreviousSequence(summary);
    var next = SelectNextSequence(summary);
    ...
    Xunit.Assert.DoesNotContain(
        typeof(global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryAdjacentSequenceSelection<
                ...
                CompletionPayload>).GetProperties(),
        property => property.SetMethod is not null);
    Xunit.Assert.Same(summary.Range, summary.Range);
}
...
public void ContractsRemainBoundedAndSideEffectFree()
{
    ...
    Xunit.Assert.Same(summary.Sequence, summary.Range.Sequence);
    Xunit.Assert.Same(summary.Chain, summary.Range.Chain);
    Xunit.Assert.Equal(summary.Range.CrossedBoundaryCount, summary.CrossedBoundaryCount);
}
