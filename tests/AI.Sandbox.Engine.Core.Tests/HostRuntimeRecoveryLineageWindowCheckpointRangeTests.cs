...
public sealed class HostRuntimeRecoveryLineageWindowCheckpointRangeTests
{
...
{
    var projection = ChainSummaryProjection();
        ...
        () => global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryLineageWindowFlow.ProjectWindow<
                ...
                CompletionPayload>(
                    ...
                    projectedTick: projection.ProjectedTick));
    ...
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => ProjectWindow(projection, startCheckpointIndex: -1));
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => ProjectWindow(projection, checkpointCount: 0));
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => ProjectWindow(projection, expectedProjectionRevision: -1));
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => ProjectWindow(projection, projectedTick: -1));

    var window = LineageWindow(projection);
        ...
        () => global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryLineageWindowFlow.QueryRange<
                ...
                CompletionPayload>(
                    ...
                    queriedTick: window.ProjectedTick));
    Xunit.Assert.Throws<ArgumentNullException>(
        () => global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryLineageWindowFlow.QueryRange<
                RequestPayload,
                ...
                CompletionPayload>(
                    ...
                    null!,
                    projection.RootCheckpointId,
                    projection.LatestCheckpointId,
    ...
    Xunit.Assert.Throws<ArgumentException>(
        () => global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryLineageWindowFlow.QueryRange<
                ...
                RecoveryWorldState,
                CompletionPayload>(
                    queryId: CheckpointRangeQueryId(),
                    window: window,
        ...
        () => global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryLineageWindowFlow.QueryRange<
                ...
                CompletionPayload>(
                    ...
                    queriedTick: window.ProjectedTick));
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => QueryRange(window, expectedWindowRevision: -1));
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => QueryRange(window, queriedTick: -1));
}

[Xunit.Fact]
public void WindowRejectsStaleRevisionAndRegressedTick()
{
    var projection = ChainSummaryProjection();

    AssertWindowStatus(
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryLineageWindowStatus.StaleProjectionRevision,
        ProjectWindow(
            projection,
            expectedProjectionRevision: projection.Revision + 1));
    AssertWindowStatus(
        global::AI.Sandbox.Engine.Core.HostRuntime
        ...
        ProjectWindow(
            projection,
            projectedTick: projection.ProjectedTick - 1));
}

[Xunit.Fact]
public void WindowRejectsInvalidBoundsAndOversizedCount()
{
    var projection = ChainSummaryProjection();

    AssertWindowStatus(
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryLineageWindowStatus.WindowStartOutOfRange,
        ProjectWindow(
            projection,
            startCheckpointIndex: projection.CheckpointCount));
    AssertWindowStatus(
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryLineageWindowStatus.WindowEndOutOfRange,
        ProjectWindow(
            projection,
...
[Xunit.Fact]
public void RootOnlyWindowPreservesBoundaryLineage()
{
    var projection = ChainSummaryProjection();
    var result = ProjectWindow(
        ...
        startCheckpointIndex: 0,
        checkpointCount: 1);
    var window = Xunit.Assert.IsType<
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryLineageWindowProjection<
                RequestPayload,
                RecoveryWorldState,
                CompletionPayload>>(result.Window);

    Xunit.Assert.True(result.Succeeded);
    Xunit.Assert.Equal(
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryLineageWindowStatus.LineageWindowProjected,
        result.Status);
    Xunit.Assert.Same(projection, result.SourceProjection);
    Xunit.Assert.Single(window.Checkpoints);
    Xunit.Assert.Empty(window.Supersessions);
    Xunit.Assert.Same(projection.Chain.RootCheckpoint, window.StartCheckpoint);
    Xunit.Assert.Same(window.StartCheckpoint, window.EndCheckpoint);
    Xunit.Assert.Null(window.IncomingSupersession);
    Xunit.Assert.Same(
        projection.Chain.FirstSupersession,
        window.OutgoingSupersession);
    Xunit.Assert.Equal(0, window.StartCheckpointIndex);
    Xunit.Assert.Equal(0, window.EndCheckpointIndex);
    Xunit.Assert.True(window.StartsAtRoot);
    Xunit.Assert.False(window.EndsAtLatest);
}

[Xunit.Fact]
public void FullWindowPreservesExactChainLineage()
{
    var projection = ChainSummaryProjection();
    var result = ProjectWindow(
        projection,
    ...
    Xunit.Assert.Equal(2, window.CheckpointCount);
    Xunit.Assert.Single(window.Supersessions);
    Xunit.Assert.Same(
        projection.Chain.RootCheckpoint,
        window.Checkpoints[0]);
    Xunit.Assert.Same(
        projection.Chain.LatestCheckpoint,
        window.Checkpoints[1]);
    Xunit.Assert.Same(
        projection.Chain.FirstSupersession,
        window.Supersessions[0]);
    Xunit.Assert.Null(window.IncomingSupersession);
    Xunit.Assert.Null(window.OutgoingSupersession);
    Xunit.Assert.True(window.StartsAtRoot);
    Xunit.Assert.True(window.EndsAtLatest);
    Xunit.Assert.Equal(projection.Revision + 1, window.Revision);
    Xunit.Assert.Equal(projection.ProjectedTick + 1, window.ProjectedTick);
}

[Xunit.Fact]
public void SingleCheckpointRangeResolvesInclusiveBounds()
{
    var window = LineageWindow();
    var result = QueryRange(
        window,
        startCheckpointId: window.StartCheckpointId,
        endCheckpointId: window.StartCheckpointId);
    ...
    Xunit.Assert.Equal(
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryLineageWindowStatus.CheckpointRangeResolved,
        result.Status);
    Xunit.Assert.Same(window, result.Window);
    Xunit.Assert.Single(query.Checkpoints);
    Xunit.Assert.Empty(query.Supersessions);
    Xunit.Assert.Same(window.StartCheckpoint, query.StartCheckpoint);
    Xunit.Assert.Same(query.StartCheckpoint, query.EndCheckpoint);
    Xunit.Assert.Null(query.IncomingSupersession);
    Xunit.Assert.Same(
        window.Supersessions[0],
        query.OutgoingSupersession);
    Xunit.Assert.Equal(0, query.StartWindowIndex);
    Xunit.Assert.Equal(0, query.EndWindowIndex);
    Xunit.Assert.Equal(window.StartCheckpointIndex, query.StartChainIndex);
    Xunit.Assert.Equal(window.StartCheckpointIndex, query.EndChainIndex);
}

[Xunit.Fact]
public void FullCheckpointRangeResolvesExactWindow()
{
    var window = LineageWindow();
    var result = QueryRange(
        window,
        startCheckpointId: window.StartCheckpointId,
        endCheckpointId: window.EndCheckpointId,
        queriedTick: window.ProjectedTick + 1);
    var query = Xunit.Assert.IsType<
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryCheckpointRangeQuery<
                RequestPayload,
    ...
    Xunit.Assert.Equal(window.CheckpointCount, query.CheckpointCount);
    Xunit.Assert.Equal(window.SupersessionCount, query.SupersessionCount);
    Xunit.Assert.Same(window.Checkpoints[0], query.Checkpoints[0]);
    Xunit.Assert.Same(window.Checkpoints[1], query.Checkpoints[1]);
    Xunit.Assert.Same(window.Supersessions[0], query.Supersessions[0]);
    Xunit.Assert.Null(query.IncomingSupersession);
    Xunit.Assert.Null(query.OutgoingSupersession);
    Xunit.Assert.Equal(0, query.StartWindowIndex);
    Xunit.Assert.Equal(1, query.EndWindowIndex);
    Xunit.Assert.Equal(window.StartCheckpointIndex, query.StartChainIndex);
    Xunit.Assert.Equal(window.EndCheckpointIndex, query.EndChainIndex);
    Xunit.Assert.Equal(window.Revision + 1, query.Revision);
    Xunit.Assert.Equal(window.ProjectedTick + 1, query.QueriedTick);
}

[Xunit.Fact]
public void RangeRejectsStaleRevisionRegressedTickAndInvalidBoundaries()
{
    var window = LineageWindow();
    var unknown = Id<
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryCheckpointIdKind>(510099);
    ...
    AssertRangeStatus(
        ...
        QueryRange(
            window,
            expectedWindowRevision: window.Revision + 1));
    AssertRangeStatus(
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryLineageWindowStatus
    ...
    AssertRangeStatus(
        ...
        QueryRange(window, startCheckpointId: unknown));
    AssertRangeStatus(
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryLineageWindowStatus.RangeEndNotFound,
        QueryRange(window, endCheckpointId: unknown));
    AssertRangeStatus(
        global::AI.Sandbox.Engine.Core.HostRuntime
...
public void ContractsRemainImmutableBoundedAndSideEffectFree()
{
    ...
    Xunit.Assert.Same(projection, window.SourceProjection);
    Xunit.Assert.Same(projection.Chain, window.SourceProjection.Chain);
    Xunit.Assert.Same(window, query.Window);
...
private static void AssertWindowStatus(
    global::AI.Sandbox.Engine.Core.HostRuntime
        .HostRuntimeRecoveryLineageWindowStatus expected,
    global::AI.Sandbox.Engine.Core.HostRuntime
            ...
            CompletionPayload> result)
{
    ...
    Xunit.Assert.Null(result.Window);
}

private static void AssertRangeStatus(
    global::AI.Sandbox.Engine.Core.HostRuntime
        .HostRuntimeRecoveryLineageWindowStatus expected,
    global::AI.Sandbox.Engine.Core.HostRuntime
