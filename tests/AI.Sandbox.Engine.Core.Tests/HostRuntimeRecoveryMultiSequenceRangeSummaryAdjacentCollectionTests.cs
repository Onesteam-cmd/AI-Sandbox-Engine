...
public sealed class HostRuntimeRecoveryMultiSequenceRangeSummaryAdjacentCollectionTests
{
...
public void IdsRevisionsTicksCountsAndArgumentsAreValidated()
{
    ...
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => ProjectMultiSequenceRangeSummary(range, expectedRangeRevision: -1));
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => ProjectMultiSequenceRangeSummary(range, projectedTick: -1));

    var summary = MultiSequenceRangeSummary(range);
    Xunit.Assert.Throws<ArgumentException>(
        () => global::AI.Sandbox.Engine.Core.HostRuntime
        ...
        () => global::AI.Sandbox.Engine.Core.HostRuntime
            ...
            .SelectPreviousCollection<
                ...
                CompletionPayload>(
                    ...
                    selectedTick: 0));
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => SelectPreviousCollection(summary, summaryCount: 0));
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => SelectPreviousCollection(summary, expectedSummaryRevision: -1));
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => SelectPreviousCollection(summary, selectedTick: -1));
}

[Xunit.Fact]
public void MultiSequenceRangeSummaryRejectsStaleRevisionAndRegressedTick()
{
    var range = MultiSequenceRange();

    AssertMultiSequenceRangeSummaryStatus(
        global::AI.Sandbox.Engine.Core.HostRuntime
...
[Xunit.Fact]
public void MultiSequenceRangeSummaryPreservesExactCompactEvidence()
{
    var range = MultiSequenceRange();
    var result = ProjectMultiSequenceRangeSummary(
        range,
        projectedTick: range.QueriedTick + 2);
    ...
    Xunit.Assert.Same(range.Collection, summary.Collection);
    ...
    Xunit.Assert.Equal(range.StartCheckpointIndex, summary.StartCheckpointIndex);
    Xunit.Assert.Equal(range.EndCheckpointIndex, summary.EndCheckpointIndex);
    Xunit.Assert.Equal(range.StartSummaryIndex, summary.StartSummaryIndex);
    Xunit.Assert.Equal(range.EndSummaryIndex, summary.EndSummaryIndex);
    ...
    Xunit.Assert.Equal(range.SummaryCount, summary.SummaryCount);
    Xunit.Assert.Equal(range.SequenceCount, summary.SequenceCount);
    Xunit.Assert.Equal(range.CrossedBoundaryCount, summary.CrossedBoundaryCount);
    Xunit.Assert.True(summary.CoversEntireCollection);
    Xunit.Assert.Equal(range.Revision + 1, summary.Revision);
    Xunit.Assert.Equal(range.QueriedTick + 2, summary.ProjectedTick);
}

[Xunit.Fact]
public void AdjacentCollectionSelectionRejectsStaleRevisionRegressedTickAndOversizedCount()
{
    var summary = MultiSequenceRangeSummary(MultiSequenceRange());

    AssertAdjacentCollectionStatus(
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryMultiSequenceCheckpointRangeSummaryStatus
    ...
    AssertAdjacentCollectionStatus(
        ...
        SelectPreviousCollection(
            ...
            summaryCount:
                global::AI.Sandbox.Engine.Core.HostRuntime
                    .HostRuntimeRecoveryMultiSequenceCheckpointRangeSummaryFlow
                    .MaximumAdjacentCollectionSummaryCount + 1));
}

[Xunit.Fact]
public void PreviousSelectionRejectsMissingAdjacentCollection()
{
    var summary = MultiSequenceRangeSummary(MultiSequenceRange());
    var result = SelectPreviousCollection(summary);
...
[Xunit.Fact]
public void NextSelectionRejectsMissingAdjacentCollection()
{
    var summary = MultiSequenceRangeSummary(MultiSequenceRange());
    var result = SelectNextCollection(summary);
...
{
    var summary = MultiSequenceRangeSummary(MultiSequenceRange());
    var previous = SelectPreviousCollection(summary);
    var next = SelectNextCollection(summary);
    ...
    Xunit.Assert.DoesNotContain(
        typeof(global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryAdjacentCollectionSelectionResult<
                ...
                CompletionPayload>).GetProperties(),
        property => property.SetMethod is not null);
    Xunit.Assert.Same(summary.Range.CrossedBoundarySupersessions,
        summary.CrossedBoundarySupersessions);
}

[Xunit.Fact]
public void FlowExposesOnlySynchronousBoundedAdvisoryOperations()
{
    var methods = typeof(global::AI.Sandbox.Engine.Core.HostRuntime
        .HostRuntimeRecoveryMultiSequenceCheckpointRangeSummaryFlow)
        .GetMethods(global::System.Reflection.BindingFlags.Public |
            global::System.Reflection.BindingFlags.Static);

    Xunit.Assert.Equal(3, methods.Length);
    Xunit.Assert.Contains(methods, method => method.Name == "ProjectSummary");
    Xunit.Assert.Contains(methods, method => method.Name == "SelectPreviousCollection");
    Xunit.Assert.Contains(methods, method => method.Name == "SelectNextCollection");
    Xunit.Assert.DoesNotContain(
        methods,
