...
public sealed class HostRuntimeRecoverySupersessionChainSelectionTests
{
...
[Xunit.Fact]
public void IdsBoundsRevisionsAndArgumentsAreValidated()
{
        ...
        () => global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoverySupersessionChainFlow.Validate<
                ...
                CompletionPayload>(
                    ...
                    validatedTick: supersession.SupersededTick + 1));
        ...
        () => global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoverySupersessionChainFlow.SelectLatest<
                ...
                CompletionPayload>(
                    ...
                    chain,
                    chain.Revision,
                    selectedTick: chain.ValidatedTick));
    Xunit.Assert.Throws<ArgumentNullException>(
        () => global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoverySupersessionChainFlow.SelectLatest<
                RequestPayload,
                ...
                CompletionPayload>(
                    ...
                    selectedTick: 0));
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => SelectLatest(chain, expectedChainRevision: -1));
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => SelectLatest(chain, selectedTick: -1));
}

[Xunit.Fact]
public void EmptyAndOversizedChainsAreRejected()
{
    AssertChainStatus(
        global::AI.Sandbox.Engine.Core.HostRuntime
...
[Xunit.Fact]
public void OptimisticSupersessionRevisionsAreExplicit()
{
    var supersession = CreateSupersession();
...
[Xunit.Fact]
public void DuplicateAndDisconnectedAuthoritiesAreRejected()
{
    var first = CreateSupersession(
        supersessionSuffix: 499101,
        checkpointSuffix: 499201,
        capturedTick: 90);
    AssertChainStatus(
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoverySupersessionChainStatus
        ...
        Validate(
            new[] { first, first },
            new[] { first.Revision, first.Revision },
            validatedTick: first.SupersededTick + 1));

    var disconnected = CreateSupersession(
        supersessionSuffix: 499102,
        checkpointSuffix: 499202,
        capturedTick: 92);
    AssertChainStatus(
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoverySupersessionChainStatus.DisconnectedChain,
        Validate(
            new[] { first, disconnected },
            new[] { first.Revision, disconnected.Revision },
            validatedTick:
                global::System.Math.Max(
                    first.SupersededTick,
                    disconnected.SupersededTick) + 1));
}

[Xunit.Fact]
public void ValidationTickAndRevisionAdvanceExactlyOnce()
{
    var supersession = CreateSupersession();
    ...
    var result = Validate(
        ...
        new[] { supersession.Revision },
        validatedTick: supersession.SupersededTick + 1);
    var chain = Xunit.Assert.IsType<
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoverySupersessionChain<
                RequestPayload,
                RecoveryWorldState,
                CompletionPayload>>(result.Chain);
    ...
    Xunit.Assert.Equal(
        ...
        result.Status);
    Xunit.Assert.Equal(supersession.Revision + 1, chain.Revision);
    Xunit.Assert.Equal(
...
[Xunit.Fact]
public void ChainValidationPreservesOrderedAuthority()
{
    var supersession = CreateSupersession();
    var result = Validate(
        new[] { supersession },
    ...
    Xunit.Assert.Single(result.Supersessions);
    Xunit.Assert.Same(supersession, result.Supersessions[0]);
    Xunit.Assert.Single(chain.Supersessions);
    Xunit.Assert.Same(supersession, chain.FirstSupersession);
    Xunit.Assert.Same(supersession, chain.LatestSupersession);
    Xunit.Assert.Same(supersession.PriorCheckpoint, chain.RootCheckpoint);
    Xunit.Assert.Same(
        supersession.SuccessorCheckpoint,
        chain.LatestCheckpoint);
    Xunit.Assert.Equal(
        supersession.RuntimeInstanceId,
        chain.RuntimeInstanceId);
    Xunit.Assert.Equal(supersession.WorldId, chain.WorldId);
}

[Xunit.Fact]
public void LatestSelectionRevisionAndTimeAreExplicit()
{
    var chain = SupersessionChain();
    ...
    AssertSelectionStatus(
        ...
        SelectLatest(
            chain,
            expectedChainRevision: chain.Revision + 1));
    AssertSelectionStatus(
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoverySupersessionChainStatus
        ...
        SelectLatest(
            chain,
            selectedTick: chain.ValidatedTick - 1));
}

[Xunit.Fact]
public void LatestSelectionReturnsExactSuccessorCheckpoint()
{
    var chain = SupersessionChain();
    var result = SelectLatest(
        chain,
        selectedTick: chain.ValidatedTick + 1);
    ...
    Xunit.Assert.Equal(
        ...
        result.Status);
    Xunit.Assert.Same(chain, result.Chain);
    Xunit.Assert.Same(chain, selection.Chain);
    Xunit.Assert.Same(chain.LatestCheckpoint, selection.Checkpoint);
    Xunit.Assert.Equal(chain.LatestCheckpointId, selection.CheckpointId);
    Xunit.Assert.Equal(
        LatestCheckpointSelectionId(),
        selection.SelectionId);
    Xunit.Assert.Equal(chain.Revision + 1, selection.Revision);
    Xunit.Assert.Equal(
        chain.ValidatedTick + 1,
        selection.SelectedTick);
}

[Xunit.Fact]
public void ContractsRemainBoundedWithoutStorageOrHistoryMutation()
{
    var chain = SupersessionChain();
    var selection = Xunit.Assert.IsType<
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryLatestCheckpointSelection<
                ...
                RecoveryWorldState,
                CompletionPayload>>(
                    SelectLatest(chain).Selection);

    Xunit.Assert.Equal(256,
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoverySupersessionChainFlow
            .MaximumSupersessionCount);
    Xunit.Assert.Same(
        chain.LatestSupersession.SuccessorCheckpoint,
