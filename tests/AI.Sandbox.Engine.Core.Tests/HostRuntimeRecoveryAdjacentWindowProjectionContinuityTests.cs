...
public sealed class HostRuntimeRecoveryAdjacentWindowProjectionContinuityTests
{
...
public void IdsRevisionsTicksAndArgumentsAreValidated()
{
        ...
        () => global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryAdjacentWindowFlow.ProjectWindow<
                ...
                CompletionPayload>(
                    ...
                    projectedTick: selection.SelectedTick));
    ...
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => ProjectAdjacentWindow(selection, projectedTick: -1));
        ...
        () => global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryAdjacentWindowFlow.ValidateContinuity<
                ...
                CompletionPayload>(
                    ...
                    null!,
                    expectedSummaryRevision: summary.Revision,
                    expectedAdjacentWindowRevision: 0,
    ...
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => ValidateContinuity(
            summary,
            projection,
            expectedAdjacentWindowRevision: -1));
    Xunit.Assert.Throws<ArgumentOutOfRangeException>(
        () => ValidateContinuity(summary, projection, validatedTick: -1));
}
...
public void ProjectionRejectsStaleRevisionAndRegressedTick()
{
    ...
    AssertProjectionStatus(
        ...
        ProjectAdjacentWindow(
            selection,
            projectedTick: selection.SelectedTick - 1));
}

[Xunit.Fact]
public void PreviousProjectionPreservesExactSelectedEvidence()
{
    var selection = PreviousSelection();
    var result = ProjectAdjacentWindow(
        selection,
        projectedTick: selection.SelectedTick + 1);
    ...
    Xunit.Assert.Equal(
        ...
        result.Status);
    Xunit.Assert.Same(selection, result.Selection);
    Xunit.Assert.Same(selection, projection.Selection);
    Xunit.Assert.Same(selection.Summary, projection.Summary);
    Xunit.Assert.Same(selection.Summary.SourceProjection, projection.SourceProjection);
    Xunit.Assert.Same(selection.Summary.Chain, projection.Chain);
    Xunit.Assert.True(projection.SelectsPreviousWindow);
    Xunit.Assert.False(projection.SelectsNextWindow);
    Xunit.Assert.Equal(selection.StartCheckpointIndex, projection.StartCheckpointIndex);
    Xunit.Assert.Equal(selection.EndCheckpointIndex, projection.EndCheckpointIndex);
    Xunit.Assert.Equal(selection.CheckpointCount, projection.CheckpointCount);
    Xunit.Assert.Single(projection.Checkpoints);
    Xunit.Assert.Empty(projection.Supersessions);
    Xunit.Assert.Same(selection.StartCheckpoint, projection.StartCheckpoint);
    Xunit.Assert.Same(selection.EndCheckpoint, projection.EndCheckpoint);
    Xunit.Assert.Same(selection.IncomingSupersession, projection.IncomingSupersession);
    Xunit.Assert.Same(selection.OutgoingSupersession, projection.OutgoingSupersession);
    Xunit.Assert.True(projection.StartsAtRoot);
    Xunit.Assert.False(projection.EndsAtLatest);
    Xunit.Assert.Equal(selection.Revision + 1, projection.Revision);
    Xunit.Assert.Equal(selection.SelectedTick + 1, projection.ProjectedTick);
}

[Xunit.Fact]
public void NextProjectionPreservesExactSelectedEvidence()
{
    var selection = NextSelection();
    var projection = AdjacentProjection(selection);

    Xunit.Assert.False(projection.SelectsPreviousWindow);
    Xunit.Assert.True(projection.SelectsNextWindow);
    Xunit.Assert.Single(projection.Checkpoints);
    Xunit.Assert.Empty(projection.Supersessions);
    Xunit.Assert.Same(selection.StartCheckpoint, projection.Checkpoints[0]);
    Xunit.Assert.Same(selection.StartCheckpoint, projection.StartCheckpoint);
    Xunit.Assert.Same(selection.EndCheckpoint, projection.EndCheckpoint);
    Xunit.Assert.Same(selection.IncomingSupersession, projection.IncomingSupersession);
    Xunit.Assert.Same(selection.OutgoingSupersession, projection.OutgoingSupersession);
    Xunit.Assert.False(projection.StartsAtRoot);
    Xunit.Assert.True(projection.EndsAtLatest);
}
...
public void ContinuityRejectsStaleRevisionsAndRegressedTick()
{
    ...
    AssertContinuityStatus(
        ...
        ValidateContinuity(
            summary,
            projection,
            expectedAdjacentWindowRevision: projection.Revision + 1));
    AssertContinuityStatus(
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryAdjacentWindowProjectionStatus
        ...
        ValidateContinuity(
            summary,
            projection,
            validatedTick: projection.ProjectedTick - 1));
}

[Xunit.Fact]
public void ContinuityRejectsMismatchedRangeSummary()
{
    var selection = PreviousSelection();
    var projection = AdjacentProjection(selection);
    var differentSummary = RangeSummary(RootRange());
...
[Xunit.Fact]
public void PreviousContinuityPreservesExactConnectingSupersession()
{
    var selection = PreviousSelection();
    var projection = AdjacentProjection(selection);
    var summary = selection.Summary;
    var result = ValidateContinuity(
        summary,
        projection,
        validatedTick: projection.ProjectedTick + 1);
    ...
    Xunit.Assert.Same(projection, result.AdjacentWindow);
    Xunit.Assert.Same(summary, validation.Summary);
    Xunit.Assert.Same(projection, validation.AdjacentWindow);
    Xunit.Assert.True(validation.ValidatesPreviousWindow);
    Xunit.Assert.False(validation.ValidatesNextWindow);
    Xunit.Assert.Same(summary.IncomingSupersession, validation.ConnectingSupersession);
    Xunit.Assert.Same(projection.EndCheckpoint, validation.PriorCheckpoint);
    Xunit.Assert.Same(summary.StartCheckpoint, validation.SuccessorCheckpoint);
    Xunit.Assert.Same(
        summary.Chain.FirstSupersession,
        validation.ConnectingSupersession);
    Xunit.Assert.Equal(
        global::System.Math.Max(summary.Revision, projection.Revision) + 1,
        validation.Revision);
    Xunit.Assert.Equal(projection.ProjectedTick + 1, validation.ValidatedTick);
}

[Xunit.Fact]
public void NextContinuityPreservesExactConnectingSupersession()
{
    var selection = NextSelection();
    var projection = AdjacentProjection(selection);
    var summary = selection.Summary;
    ...
    Xunit.Assert.False(validation.ValidatesPreviousWindow);
    Xunit.Assert.True(validation.ValidatesNextWindow);
    Xunit.Assert.Same(summary.OutgoingSupersession, validation.ConnectingSupersession);
    Xunit.Assert.Same(summary.EndCheckpoint, validation.PriorCheckpoint);
    Xunit.Assert.Same(projection.StartCheckpoint, validation.SuccessorCheckpoint);
    Xunit.Assert.Same(
        summary.Chain.FirstSupersession,
        validation.ConnectingSupersession);
}
...
public void ContractsRemainImmutableBoundedAndSideEffectFree()
{
    ...
    Xunit.Assert.Equal(
        64,
        global::AI.Sandbox.Engine.Core.HostRuntime
            .HostRuntimeRecoveryAdjacentWindowFlow.MaximumCheckpointCount);
    Xunit.Assert.Same(selection, projection.Selection);
    Xunit.Assert.Same(selection.Summary, validation.Summary);
