# ADR-0076: Host recovery multi-collection-sequence-sequence-sequence checkpoint-range summary and adjacent multi-collection-sequence-sequence selection contracts

- **Status:** Accepted
- **Date:** 2026-08-06
- **Scope:** Immutable checkpoint-range summary projection and bounded adjacent multi-collection-sequence-sequence selection authorities

## Context

Commit 0075 validates one exact continuous multi-collection-sequence-sequence sequence and resolves one bounded checkpoint range crossing one or more validated multi-collection-sequence-sequence boundaries. Callers can inspect the complete immutable query authority, but Core still needs one compact projection of that exact range and one explicit advisory authority selecting a caller-sized multi-collection-sequence-sequence immediately before or after it.
...
`ProjectSummary` accepts one externally assigned summary identity, one exact `HostRuntimeRecoveryMultiCollectionSequenceSequenceSequenceCheckpointRangeQuery`, its expected revision, and an external projection tick. It preserves the unchanged query, validation sequence, source authorities, checkpoints, crossed boundaries, indexes, optimistic revision, and monotonic time.

`SelectPreviousMultiCollectionSequenceSequence` and `SelectNextMultiCollectionSequenceSequence` accept one externally assigned selection identity, one exact summary, a positive bounded count, its expected revision, and an external selection tick. They select exact `HostRuntimeRecoveryContinuousMultiCollectionSequenceSequenceSummaryProjection` authorities immediately adjacent to the summarized interval, preserve source order and every internal boundary, and retain the exact supersession connecting selection and range.
...
- adjacent selection retains exact multi-collection-sequence-sequence summaries, internal boundaries, source authorities, endpoints, aggregate evidence, revision, and external tick;
