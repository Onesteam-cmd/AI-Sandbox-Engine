# ADR-0065: Host recovery adjacent collection-sequence projection and multi-collection checkpoint-range continuity validation contracts

- **Status:** Accepted
- **Date:** 2026-08-05
- **Scope:** Immutable adjacent collection-sequence projection and exact multi-collection checkpoint-range continuity-validation authorities

## Context

Commit 0064 projects one exact bounded multi-collection checkpoint-range summary
and selects one exact previous or next adjacent collection sequence from the same
validated source sequence. Callers can inspect the selection authority, but Core
still needs an explicit materialized projection of its exact checkpoints and
supersessions and an authority proving immediate collection-pair, checkpoint,
...
`ProjectCollectionSequence` accepts one exact
`HostRuntimeRecoveryAdjacentCollectionSequenceSelection`, its expected revision,
and an external projection tick. It verifies the exact selected collection-pair
references, internal and adjacent boundary supersessions, checkpoint interval,
and incoming/outgoing supersessions against the immutable source sequence and
chain before materializing exact read-only checkpoint and supersession arrays.

`ValidateContinuity` accepts one exact
`HostRuntimeRecoveryMultiCollectionCheckpointRangeSummaryProjection`, one exact
projected adjacent collection sequence, both expected revisions, and an external
validation tick. It requires both authorities to share the same source summary,
collection-pair intervals and checkpoint intervals to be immediately adjacent,
and the same exact connecting supersession with matching endpoint checkpoints.
...
- continuity validation preserves the exact range summary, projected sequence,
  selection, connecting supersession, endpoint checkpoints, external tick, and
  optimistic revision;
