# ADR-0069: Host recovery adjacent multi-collection projection and multi-collection-sequence checkpoint-range continuity validation contracts

- **Status:** Accepted
- **Date:** 2026-08-06
- **Scope:** Immutable adjacent multi-collection projection and exact multi-collection-sequence checkpoint-range continuity-validation authorities

## Context

Commit 0068 projects one exact bounded multi-collection-sequence checkpoint-range
summary and selects one exact previous or next adjacent multi-collection from the
...
supersessions and an authority proving immediate multi-collection, checkpoint,
...
`ProjectMultiCollection` accepts one exact
`HostRuntimeRecoveryAdjacentMultiCollectionSelection`, its expected revision,
and an external projection tick. It verifies the exact selected multi-collection
...
one exact projected adjacent multi-collection, both expected revisions, and an
external validation tick. It requires both authorities to share the same source
summary, multi-collection intervals and checkpoint intervals to be immediately
...
- continuity validation preserves the exact range summary, projected
  multi-collection, selection, connecting supersession, endpoint checkpoints,
