# ADR-0067: Host recovery continuous multi-collection sequence validation and multi-collection-sequence checkpoint-range query contracts
...
Commit 0066 projects one exact continuous multi-collection summary over a
...
multi-collection summaries, but Core still needs an explicit authority proving
that their caller-supplied order is continuous and one bounded query authority
over ranges crossing one or more validated collection-sequence boundaries.
...
`HostRuntimeRecoveryContinuousMultiCollectionSummaryProjection` authorities,
...
- callers receive one immutable proof over an ordered bounded sequence of exact
  continuous multi-collection summaries;
