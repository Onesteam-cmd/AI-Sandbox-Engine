...
/// <summary>
/// Represents one stable machine-readable prompt-composition rejection reason.
/// </summary>
public readonly record struct PromptRejectionCode :
    IComparable<PromptRejectionCode>
{
    private const int MaximumLength = 64;
    private readonly string? value;

    private PromptRejectionCode(string value)
    {
        this.value = value;
    }
    ...
    public string Value =>
        ...
        throw new InvalidOperationException(
            "The prompt rejection code is not initialized.");
    ...
    public static PromptRejectionCode Parse(string text)
    {
        if (!TryParse(text, out var code))
        {
            throw new FormatException(
                "Prompt rejection codes must contain 1 to 64 lowercase ASCII " +
    ...
    public static bool TryParse(string? text, out PromptRejectionCode code)
    {
        if (!IsValid(text))
        {
            code = default;
        ...
        code = new PromptRejectionCode(text!);
        return true;
    }

    /// <inheritdoc />
    public int CompareTo(PromptRejectionCode other)
    {
        if (!IsInitialized)
        {
