...
/// <summary>
/// Represents one stable machine-readable reason for rejecting retrieval.
/// </summary>
public readonly record struct ContextRejectionCode :
    IComparable<ContextRejectionCode>
{
    private const int MaximumLength = 64;
    private readonly string? value;

    private ContextRejectionCode(string value)
    {
        this.value = value;
    }
    ...
    public string Value =>
        ...
        throw new InvalidOperationException(
            "The context rejection code is not initialized.");
    ...
    public static ContextRejectionCode Parse(string text)
    {
        if (!TryParse(text, out var code))
        {
            throw new FormatException(
                "Context rejection codes must contain 1 to 64 lowercase ASCII " +
        ...
        out ContextRejectionCode code)
    {
        ...
        code = new ContextRejectionCode(text!);
        return true;
    }

    /// <inheritdoc />
    public int CompareTo(ContextRejectionCode other)
    {
        if (!IsInitialized)
        {
